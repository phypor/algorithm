...
public class AppController {
	...
	public ResponseEntity<Object> download(String fileName) throws IOException {
		...
		return new ResponseEntity<Object>(
				FileUtils.readFileToByteArray(new File(filePath + fileName)),
	...
	public String management(
		...
		List<AppChannelType> appChannelTypeList = appChannelTypeService
				.getAll(AppChannelType.class);

		List<AppPlatformType> appPlatformTypeList = appPlatformTypeService
				.getAll(AppPlatformType.class);
		...
		Pagination<AppClientInfo> recruitInfoPagination = appClientInfoService
				.getPagination(AppClientInfo.class, queryConditions,
	...
	public Map<String, Object> addAppClient(HttpServletRequest request,
			String pushTime, @Valid AppClientInfo appClientInfo,
		...
		appClientInfo.setPushTime(DateUtil.parseDateByString(pushTime,
				"yyyy-MM-dd"));
	...
	public Model checkRenewAppClient(
			@RequestParam(required = false, defaultValue = "1") Long appChannelTypeId,
			@RequestParam(required = false, defaultValue = "0") double versionCode,
			@RequestParam(required = false, defaultValue = "1") Long appPlatformTypeId,
			Model model) throws Exception {

		AppClientInfo appClientInfo = (AppClientInfo) appClientInfoService
				.findByChannelPlatformTypeVersion(appChannelTypeId,
						versionCode, appPlatformTypeId);
	...
	public Map<String, Object> addFeedBackLog(Model model,
			HttpServletRequest request, @Valid FeedBackLog feedBackLog,
			BindingResult res) throws Exception {
		User currentUser = (User) request.getSession().getAttribute(
				PublicInfoConfig.currentUser);
