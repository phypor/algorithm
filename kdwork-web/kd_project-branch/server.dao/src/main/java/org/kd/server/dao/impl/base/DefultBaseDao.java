package org.kd.server.dao.impl.base;
...
public class DefultBaseDao implements IBaseDao {
	...
	private Query getQuery(Class clazz, List<QueryCondition> queryConditions,
			String orderBy, boolean isQueryTotal)throws Exception {
		String className = clazz.getSimpleName();
			...
			while (iterator2.hasNext()) {
				...
				if (queryCondition != null) {
					if (queryCondition.getValue() != null
						...
						if (queryCondition.getOperator().equals(
								QueryCondition.LK)) {
							query.setParameter(placeholder, "%"
	...
	public  Query createQuery(String jpql, Object... objects)throws Exception {   
        Query query =em.createQuery(jpql);   
        if  (objects !=  null ) {   
            for  ( int  i =  0 ; i < objects.length; i++) {   
                query.setParameter(i, objects[i]);   
	...
	public <T>List<T>  pageQuery(int currentPage, int pageSize,String jpql,Object... objects)throws Exception{
	...
	public <T>List<T> getPageQuery(int currentPage, int pageSize,
			String jpql, Object... objects)throws Exception {
